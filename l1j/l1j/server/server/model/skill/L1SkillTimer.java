...
class L1SkillStop {
	...
	public static void stopSkill(L1Character cha, int skillId) {
		...
		}else if (skillId == 2500) {  //銀行利息
			L1PcInstance pc = (L1PcInstance) cha;
			if ((cha instanceof L1PcInstance) && pc.getBank() <= 190000000) {
				int interest = pc.getBank() / Config.INTEREST_RATE; //銀行倍率設置
				pc.setBank(pc.getBank() + interest);
			}
			pc.setSkillEffect(2500, Config.BANK_INTEREST * 1000);
		}
