...
class NameIdMaker {
	...
	public static void main(String args[]) {
		try {
			if (args.length < 1) {
				J_Main.getInstance().addConsolPost("引数が足りません。");
				J_Main.getInstance().addConsolPost("USAGE: inputfile [language]");
				return;
			}
			if (args.length >= 2) {
				loadTable(args[1]);
			...
			J_Main.getInstance().addConsolPost(e.getLocalizedMessage());
		}
	}
...
//						&& line.length() < 24 // アイテムやNPCの名前とは思えない
	...
	public static void convert(String inputpath) throws IOException {
		...
		try {
			...
			inr = new LineNumberReader(new FileReader(file));
			String line;
			J_Main.getInstance().addConsolPost(inputpath + " -> " + outputpath + " ... ");
			while ((line = inr.readLine()) != null) {
				pw.println(getReplacedString(line, false));
			}
			J_Main.getInstance().addConsolPost(inr.getLineNumber() + "件を出力しました。");
		} catch (FileNotFoundException e) {
