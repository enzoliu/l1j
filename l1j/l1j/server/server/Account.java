...
public class Account {
	...
	public void updAccountWarehouse(){
		ArrayList<String> pc_name = getCharacters();
		for(L1PcInstance pc : L1World.getInstance().getAllPlayers()){
			for(int i=0;i<pc_name.size();i++){
				if(pc.getName().equals(pc_name.get(i))){
					pc.getDwarfInventory().clearItems();
					pc.getDwarfInventory().loadItems();
				}
			}
		}
	}
	public boolean isAccountWarehouseLock(){
		boolean lock = false;
		ArrayList<String> pc_name = getCharacters();
		for(L1PcInstance pc : L1World.getInstance().getAllPlayers()){
			for(int i=0;i<pc_name.size();i++){
				if(pc.getName().equals(pc_name.get(i))){
					if(pc.isWarehouseLock()){
						lock = true;
						break;
					}
				}
	...
	private boolean _is_dwarf_load = false;
	public boolean is_dwarf_load(){
		return _is_dwarf_load;
	}
	public void loadDwarfInventory(){
		ArrayList<String> pc_name = getCharacters();
		L1PcInstance pc = L1PcInstance.load(pc_name.get(0));
		_dwarf = pc.getDwarfInventory();
		_dwarf.loadItems();
		_is_dwarf_load = true;
	}
	public L1DwarfInventory getWarehouse(){
		return _dwarf;
	}
	/**
	 * キャラクター所有数をカウントする.
	...
	//太古的玉璽 實裝 (給玩家增加帳號欄位用) by seroidv
