...
public class DropTable {
	...
	public void resetDrop(L1NpcInstance npc, L1Inventory inventory, L1PcInstance pc){
		int mobId = npc.getNpcTemplate().get_npcId();
		ArrayList<L1Drop> dropList = _droplists.get(mobId);
		if (dropList == null) {
			return;
		...
		for (L1Drop drop : dropList) {
			...
			double rateOfMapId = MapsTable.getInstance().getDropRate(npc.getMapId());
			double rateOfItem = DropItemTable.getInstance().getDropRate(itemId);
			double totalDrop = drop.getChance() * droprate * rateOfMapId * rateOfItem;
			if(pc.hasSkillEffect(7003)){
				totalDrop *= 5;
			}
			if (droprate == 0 || totalDrop < randomChance) {
				continue;
			}
