	...
	public void onTarget() {
		...
		if (getAtkspeed() == 0) { // 逃げるキャラ
			...
			if (isAttackPosition(target.getX(), target.getY(), getNpcTemplate()
				...
				if (mobSkill.skillUse(target, false)) { // スキル使用(mobskill.sqlのTriRndに従わず、発動確率は100%)
					setSleepTime(calcSleepTime(mobSkill.getSleepTime(),
							MAGIC_SPEED));
					return;
				}
	...
	public void attackTarget(L1Character target) {
		...
		if (this instanceof L1PetInstance) {
			...
			if (target instanceof L1NpcInstance) {
				L1NpcInstance tg = (L1NpcInstance) target;
				if (this.getMaster() == tg.getMaster()) {
					return;
				}
			} else if (this.getMaster() == target) {
				return;
			}
