...
public class L1V1Map extends L1Map {
	...
	int newX[] = {-1,0,1,-1,0,1,-1,0,1};
	int newY[] = {1,1,1,0,0,0,-1,-1,-1};
	
	@Override
	public boolean isPassable2(int x, int y) {
		//int tile = accessTile(x, y);
		//if(Config.OPCODE)
		//	L1World.getInstance().broadcastServerMessage("tile:"+tile);
		//if(tile==0){
		/*
			for(int i=0;i<9;i++){
				tile = accessTile(x+newX[i], y+newY[i]);
