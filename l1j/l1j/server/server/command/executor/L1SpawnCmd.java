...
public class L1SpawnCmd implements L1CommandExecutor {
	...
	private void sendErrorMessage(L1PcInstance pc, String cmdName) {
		String errorMsg = cmdName + " npcid|name [數量] [範圍] ←請輸入。 ";
	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		try {
			...
			if (npc == null) {
				pc.sendPackets(new S_SystemMessage("找不到符合條件NPC。"));
				return;
			}
			for (int i = 0; i < count; i++) {
				L1SpawnUtil.spawn(pc, npcid, randomrange, 0);
			}
			String msg = String.format("%s(%d) (%d) 召喚回來。 (範圍:%d)", npc
			...
			pc.sendPackets(new S_SystemMessage(cmdName + " 內部錯誤。"));
		}
	}
}
