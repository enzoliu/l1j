...
public class L1Magic {
		...
		if (_pc != null && _targetPc != null) {
			if (_pc.getZoneType() == 1 || _targetPc.getZoneType() == 1) { // セーフティーゾーン
				if (skillId == WEAPON_BREAK || skillId == SLOW || skillId == CURSE_PARALYZE || skillId == MANA_DRAIN
						...
						|| skillId == ICE_LANCE || skillId == FREEZING_BLIZZARD || skillId == FREEZING_BREATH
	...
	private int calcProbability(int skillId) {
		...
		if (skillId == ELEMENTAL_FALL_DOWN || skillId == RETURN_TO_NATURE || skillId == ENTANGLE
				|| skillId == ERASE_MAGIC || skillId == AREA_OF_SILENCE || skillId == WIND_SHACKLE
		...
		} else if (skillId == SHOCK_STUN || skillId == COUNTER_BARRIER) {
			// 成功確率は 基本確率 + LV差1毎に+-1%
	...
	private static final int[] INVINCIBLE = { ABSOLUTE_BARRIER, ICE_LANCE, FREEZING_BLIZZARD, FREEZING_BREATH,
			EARTH_BIND };

	/* ■■■■■■■■■■■■■■ 魔法ダメージ算出 ■■■■■■■■■■■■■■ */

	public int calcMagicDamage(int skillId, L1PcInstance pc) {
		int damage = 0;
		for (int i = 0; i< INVINCIBLE.length; i++) {
			if (_targetPc != null) {
				if (_targetPc.hasSkillEffect(INVINCIBLE[i])) {
					return damage;
				}
			} else if (_targetNpc != null) {
				if (_targetNpc.hasSkillEffect(INVINCIBLE[i])) {
					return damage;
				}
			}
		}
		...
		if (_calcType == PC_PC || _calcType == PC_NPC) {
			damage *= 1 + (pc.getInt() / 200);
			if (Config.HIDDEN_ATTR_ENABLE) {
				damage += pc.getAdditionalAttr(4);
			}
		}		
	...
	private int calcPcMagicDamage(int skillId) {
		...
		if (_targetPc.hasSkillEffect(COOKING_1_0_S) // 料理によるダメージ軽減
				...
				|| _targetPc.hasSkillEffect(COOKING_2_2_S) || _targetPc.hasSkillEffect(COOKING_2_3_S)
				|| _targetPc.hasSkillEffect(COOKING_2_4_S) || _targetPc.hasSkillEffect(COOKING_2_5_S)
				|| _targetPc.hasSkillEffect(COOKING_2_6_S) || _targetPc.hasSkillEffect(COOKING_3_0_S)
				|| _targetPc.hasSkillEffect(COOKING_3_1_S) || _targetPc.hasSkillEffect(COOKING_3_2_S)
				|| _targetPc.hasSkillEffect(COOKING_3_3_S) || _targetPc.hasSkillEffect(COOKING_3_4_S)
	...
	private int calcNpcMagicDamage(int skillId) {
		...
		if (skillId == FINAL_BURN) {
			...
			if (skillId == JOY_OF_PAIN) {
				_pc.sendPackets(new S_SystemMessage("計算Dice damage"));				
			}
			dmg = calcMagicDiceDamage(skillId);
			dmg = (dmg * getLeverage()) / 10;
		}
		...
		if (skillId == JOY_OF_PAIN) {
			//TODO: remove test block
			_pc.sendPackets(new S_SystemMessage("進入計算疼痛歡愉區塊"));		
			int nowDamage = 0;
			if (_calcType == PC_NPC) {
				nowDamage = _pc.getMaxHp() - _pc.getCurrentHp();
				if (nowDamage > 0) {
			...
			//TODO: remove test block
			_pc.sendPackets(new S_SystemMessage("計算結果" + Integer.toString(dmg)));
		}
		...
		//TODO: remove test block
		if (skillId == JOY_OF_PAIN) {
			_pc.sendPackets(new S_SystemMessage("即將進入ice cave計算"));				
		}
	...
	public int calcHealing(int skillId) {
		...
		if (Config.HIDDEN_ATTR_ENABLE) {
			magicDamage += _pc.getAdditionalAttr(4);
		}
	...
	private static void IceCave(L1NpcInstance npc, double dmg) {
		...
		for (int i = 0; i < npcId.length; i++) {
			if (npc.getNpcTemplate().get_npcId() == npcId[i] && dmg >= npc.getCurrentHp()) {
