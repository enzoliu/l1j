...
public class NpcTable {
	...
	//所有怪物都可抓進化
	private final HashMap<Integer, Integer> _ericNpcLevel = new HashMap<Integer, Integer>();
	//~所有怪物都可抓進化
	...
	private void loadNpcData() {
		...
		try {
			...
			//所有怪物都可抓進化
			pstm = con.prepareStatement("select a1.lvl,COUNT(*) cont from npc a1,npc a2 where a1.lvl=a2.lvl and a1.npcid=a2.npcid and a1.impl='L1Monster' GROUP BY a1.lvl");
			rs = pstm.executeQuery();
			while (rs.next()) {
				int level = rs.getInt("lvl");
				int cont = rs.getInt("cont");
				_ericNpcLevel.put(level, cont);
			}
			//~所有怪物都可抓進化
	...
	//所有怪物都可抓進化
	public int findNpcIdByLevel(int level) {
		boolean fail=false;
		//等級不等於MaxCEPetLevel級  且 ...
		Random random = new Random();
		while(level<(Config.MAX_CE_PET_LEVEL+1) && !fail){
			Integer cont = _ericNpcLevel.get(level);
			//這等級沒這怪物 或者是boss  則等級+1
			while(cont==null){
				cont = _ericNpcLevel.get(level+=1);
				//等級等於MaxCEPetLevel級 則準備跳出
				if(level>=(Config.MAX_CE_PET_LEVEL+1)){
					fail=true;
					break;
				}
			}
			...
			//有這怪物
			int select = random.nextInt(cont)+1;
			int tempCont=0;
			for (L1Npc npc : _npcs.values()) {
				//外加不等於boss
				if (npc.get_level()==level && !BossSpawnTable.getInstance().isBoss(npc.get_npcId())) {
					tempCont+=1;
				}
				if(tempCont==select ){
					return npc.get_npcId();
