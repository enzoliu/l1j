...
public class L1War {
	...
	class SPEWarTimer implements Runnable {
		public SPEWarTimer(){
		}
		
		@Override
		public void run(){
			for (int loop = 0; loop <= Config.RESTART_TIME; loop++) { // 240分
				try {
					Thread.sleep(60000);
				} catch (Exception exception) {
					break;
	...
	private void RequestSPEWar(int type, String clan1_name, String clan2_name) {
		//never end war add by sdcom 2009.12.2
		if (clan1_name == null || clan2_name == null) {
			return;
		}
	...
	public void DeclareWarSpe(String clan1_name, String clan2_name){
		RequestSPEWar(1, clan1_name, clan2_name);
	}
