...
public class C_Attr extends ClientBasePacket {
	...
	public C_Attr(byte abyte0[], ClientThread clientthread) throws Exception {
			...
			if (readC() == 1) {
				...
				if (addition > -1) {
					if (Config.HIDDEN_ATTR_ENABLE) {
						// 條件:全屬性加滿後才進入隱藏屬性加成
						if (pc.getBaseStr() + pc.getBaseCon() + pc.getBaseDex() + pc.getBaseCha() + pc.getBaseInt() + pc.getBaseWis() < 210) {
							pc.sendPackets(new S_SystemMessage("屬性最大數值只能到"+Config.MAX_ABILITY+"。請重試一次。"));
							break;
						} else {
							pc.setAdditionalAttr(addition, pc.getAdditionalAttr(addition) + 1);
						}
					} else {
						pc.sendPackets(new S_SystemMessage("屬性最大數值只能到"+Config.MAX_ABILITY+"。請重試一次。"));
						break;
		...
		//XXX 寵物競速 預約名單回應
		case 1256:
			l1j.expand.L1PolyRace.getInstance().requsetAttr(pc, readC());
			break;
		//END
		default:
			break;
