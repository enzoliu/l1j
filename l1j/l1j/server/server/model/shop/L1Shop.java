...
public class L1Shop {
	...
	public List<L1AssessedItem> allAssessItems(L1PcInventory inv) {
		...
		for (L1ItemInstance targetItem : inv.getItems()) {
			...
			if(ShopTable.getInstance().getItemPrice(targetItem.getItemId())>0){
				result.add(new L1AssessedItem(targetItem.getId(), ShopTable.getInstance().getItemPrice(targetItem.getItemId())));
			}
	...
	public L1AssessedItem allAssessItem(L1ItemInstance item) {
		return new L1AssessedItem(item.getId(),	ShopTable.getInstance().getItemPrice(item.getItemId()));
	}
	//~新所有物品販賣 by eric1300460
	//~修正新所有東西販賣
	/**
	...
	public void buyItems(L1ShopSellOrderList orderList) {
		...
		int e = 100000000;
		if(totalPrice>e){
			ePrice = (totalPrice-(totalPrice%e))/e;
			totalPrice = totalPrice%e;
		}
		L1ItemInstance adena = inv.findItemId(L1ItemId.ADENA);
		if(adena.getCount()+totalPrice>=20*e){
			ePrice += (adena.getCount()-(adena.getCount()%e))/e;
			totalPrice += adena.getCount()%e;
			inv.deleteItem(adena);
		}
		if(ePrice>0){
			inv.storeItem(300009,ePrice);
		}
