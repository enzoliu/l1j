...
public class C_NPCAction extends ClientBasePacket {
	...
	public C_NPCAction(byte abyte0[], ClientThread client) throws Exception {
        ...
        try{
            ...
            System.out.println("GM 功能選單 error!");
        }
        //~GM指令
		//~加入GM HTML
		...
		if (obj != null) {
			if (obj instanceof L1NpcInstance) {
				...
				// 判斷是否施法
				if (s.matches("[0-9]+")) {  
					if (target.isSummonMonster() || target.isGm() || Config.ALL_GM) { // 判斷是否施法
					...
					if (target.isShapeChange() || target.isGm() || Config.ALL_GM) { // 判斷是否施法
		...
		//寵物競速	//90003 NPC的編號
		} else if(((L1NpcInstance)obj).getNpcTemplate().get_npcId()==100000  
			&& s.equalsIgnoreCase("ent")){
			l1j.expand.L1PolyRace.getInstance().enterGame(pc);
		//END
		...
		} else if (s.equalsIgnoreCase("get")) {//sdcom參考
			L1NpcInstance npc = (L1NpcInstance) obj;
		...
		// タウンアドバイザー
			...
			if (s.equalsIgnoreCase("r")) {
				if (obj instanceof L1NpcInstance) {
					//no use ,denote by sdcom 2010.9.9
					//int npcid = ((L1NpcInstance) obj).getNpcTemplate().get_npcId();
					//int town_id = L1TownLocation.getTownIdByNpcid(npcid);
				}
			}
	...
	private boolean usePolyScroll(L1PcInstance pc, int item_id, String s) {
		int time = 0;
		if (item_id == 40088 || item_id == 40096) { // ?身?????、象牙?塔??身?????
			time = 1800;
		} else if (item_id == 140088) { // 祝福????身?????
			time = 2100;
		}
