		...
		try {
			...
			if (target != null) {
				pc.sendPackets(new S_SystemMessage((new StringBuilder())
						.append(target.getName()).append(" 您被GM強制剔除遊戲。")
						.toString()));
				// SKTへ移動させる
				target.setX(33080);
				target.setY(33392);
				...
				targetClient.kick();
				_log.warning("GM的skick指令使得 (" + targetClient.getAccountName()
						+ ":" + targetClient.getHostname() + ") 的連線被強制中斷。");
			} else {
				pc.sendPackets(new S_SystemMessage(
						"此玩家不在線上。"));
			}
		} catch (Exception e) {
			pc.sendPackets(new S_SystemMessage(cmdName + " 角色名稱 ←請輸入。 "));
		}
	}
}
