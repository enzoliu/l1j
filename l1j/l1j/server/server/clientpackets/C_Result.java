...
public class C_Result extends ClientBasePacket {
	...
	public C_Result(byte abyte0[], ClientThread clientthread)throws Exception {
		...
		int size = readC();
		readC();//unknown fix by sdcom 2010.9.9
		...
		//add by sdcom 2010.3.12
		if(resultType == 3 && npcImpl.equalsIgnoreCase("L1Dwarf")){
			pc.unlockWarehouse();
		}
		...
		} else if (resultType == 1 && size != 0	&& npcImpl.equalsIgnoreCase("L1Merchant")) { // アイテム売却
			L1Shop shop = ShopTable.getInstance().get(npcId);
			L1ShopSellOrderList orderList = shop.newSellOrderList(pc);
			for (int i = 0; i < size; i++) {
				itemId = readD();
				orderList.add(itemId, readD());
			}
