...
public class MySqlCharactersItemStorage extends CharactersItemStorage {
	...
	public void storeItem(int objId, L1ItemInstance item) throws Exception {
		...
		try {
			...
			//屬性強化捲軸
			pstm = con
					.prepareStatement("INSERT INTO character_items SET id = ?, item_id = ?, char_id = ?, item_name = ?, count = ?, is_equipped = 0, enchantlvl = ?, is_id = ?, durability = ?, charge_count = ?, remaining_time = ?, last_used = ?, attr = ?, attrlvl = ?, firemr = ?, watermr = ?, earthmr = ?, windmr = ?, addsp = ?, addhp = ?, addmp = ?, hpr = ?, mpr = ?, gethp = ?, getmp = ?, bless = ?");
	...
	//屬性強化捲軸
	@Override
	public void updateItemAttribute(L1ItemInstance item) throws Exception {
		Connection con = null;
		PreparedStatement pstm = null;
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con
					.prepareStatement("UPDATE character_items SET attr = ? WHERE id = ?");
			pstm.setInt(1, item.getAttribute());
	...
	@Override
	public void updateItemAttributeLevel(L1ItemInstance item) throws Exception {
		Connection con = null;
		PreparedStatement pstm = null;
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con
					.prepareStatement("UPDATE character_items SET attrlvl = ? WHERE id = ?");
			pstm.setInt(1, item.getAttributeLevel());
			pstm.setInt(2, item.getId());
			pstm.execute();
		} catch (SQLException e) {
			throw e;
...
//	飾品強化卷軸
	...
	//武器吸血吸魔魔法卷軸 by eric1300460
	@Override
	public void updateGetHp(L1ItemInstance item) throws Exception {
		executeUpdate(item.getId(),
				"UPDATE character_items SET gethp = ? WHERE id = ?", item
						.getgetHp());
		item.getLastStatus().updateGetHp();
	}
	@Override
	public void updateGetMp(L1ItemInstance item) throws Exception {
		executeUpdate(item.getId(),
				"UPDATE character_items SET getmp = ? WHERE id = ?", item
						.getgetMp());
		item.getLastStatus().updateGetMp();
	}
	//~武器吸血吸魔魔法卷軸 by eric1300460
	@Override
