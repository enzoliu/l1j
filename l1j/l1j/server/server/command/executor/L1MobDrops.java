...
public class L1MobDrops implements L1CommandExecutor {
	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		...
		try {
			...
			int mobID = Integer.parseInt(stringtokenizer.nextToken());
			...
			pc.sendPackets(new S_SystemMessage(theMob.get_name() + "(" + mobID + ") | gfxID: " + gfxID + " | 掉落查詢:"));
			for (L1Drop drop : mobDrops) {
				...
				pc.sendPackets(new S_SystemMessage(blessed + "數量: " + drop.getMin() + "~" + drop.getMax() + " 物品: " + drop.getItemid() + " " + item.getName() + " 機率: " + chance + "%"));
			}
		} catch(Exception e) {
			pc.sendPackets(new S_SystemMessage("請輸入 ." + cmdName + "[NPC編號]。"));
		} finally {
			SQLUtil.close(rs);
			SQLUtil.close(pstm);
			SQLUtil.close(con);
